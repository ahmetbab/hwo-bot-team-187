    ...
    public Vector3 myOffensiveEval(double timeLeft, PongGameBot bot, ClientGameState state, PongGameBot.PlayerSide catcher, double catcherPos, ClientGameState.Ball collidingBallState, ClientGameState.Ball tmpBall, double minVal, double maxVal) {
...

        {
            for(int i=3; i<97; ++i) {
                ...
                if(bot.hasMissiles() && moveTime * bot.getPaddleMaxVelocity() > missileDistance + 30) {
                    tmpTopValue = 1000000 - missileDistance+tmpTopValue; // moveTime * bot.getPaddleMaxVelocity(); // should be an easy win.
                    tmpBotValue = 1000000 -missileDistance+tmpBotValue; // moveTime * bot.getPaddleMaxVelocity(); // should be an easy win.
                }
